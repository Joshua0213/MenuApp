...
import { setDisplayBrightness } from "../../../../../actions/menubuilderActions";

class Brightnesstoggle extends Component {
  constructor(props) {
    super(props);
    this.toggleBrightness = this.toggleBrightness.bind(this);
  }

  toggleBrightness() {
    if (this.props.menuArr.displayBrightness === "light") {
      this.props.setDisplayBrightness("dark");
    } else {
      this.props.setDisplayBrightness("light");
    }
  }

  render() {
    let { displayBrightness, displaySize } = this.props.menuArr;
    let content;
    let iconClasses = "w-6 h-6 fill-current";
    let toggleClick = this.toggleBrightness;
...
export default connect(mapStateToProps, { setDisplayBrightness })(
  Brightnesstoggle
);
